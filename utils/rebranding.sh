...
REPLACE_FILES=$(find . -not -path "./vendor/*" \
    ...
    -exec grep -L "Copyright (c) 2026 Proton AG" {} \;)
...
do
    ...
    "s/along with ProtonMail Bridge.  If not, see/along with Proton Mail Bridge. If not, see/" \
    "s/Copyright (c) 2022 Proton Technologies AG/Copyright (c) 2026 Proton AG/" \
    "s/Copyright (c) 2021 Proton Technologies AG/Copyright (c) 2026 Proton AG/" \
    "s/Copyright (c) 2020 Proton Technologies AG/Copyright (c) 2026 Proton AG/"
    do
        sed -i "$repl" "$f" || exit 3
    done
done
